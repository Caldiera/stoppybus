...
shotgun = pygame.mixer.Sound(os.path.join("Sound","shotgun.wav"))
...
#______Background music_____________________________________
pygame.mixer.music.load(os.path.join("Sound","happy.mp3"))


# ___________Stuff for tic tack toe_______________________________________________tick tack toe stuff...
BOARD_ROWS=3
BOARD_COLUMNS=3
victory=0
player=1
...
BUS_IMG = pygame.image.load(
    os.path.join('Images','bus.png'))
ROAD_IMG = pygame.image.load(
    os.path.join('Images','road.png'))
SCOREBOARD_IMG = pygame.image.load(
    os.path.join('Images','billboard.png'))
EXIT_IMG = pygame.image.load(
    os.path.join('Images','exit.png'))
...
#_________Fonts and text_____________________________________________________________Writing Functions...
...
def draw_menu_bar(menu_bar_layer,game_mode_layer,time_attack_layer,classic_layer):
    #pygame.draw.rect(win,MENU_BG,menu_bar_layer,border_radius=15)
...
#________ Writing the text that appears on screen__________
    ...
    while run ==True:
        ...
        for event in pygame.event.get(): #looks for any event (most are built in already defined by pygame (can have User defined events as well))
            ...
            if event.type==pygame.MOUSEBUTTONDOWN:
                ...
                if time_attack_layer.collidepoint(event.pos):
                    crash.play()
                    time_attack=True
                    pygame.time.set_timer(pygame.USEREVENT, 1000)
                    counter, text = 30, '30'.rjust(3)

                if classic_layer.collidepoint(event.pos):
                    classic=True
                    crash.play()
                ...
                if game_board_layer.collidepoint(event.pos) and exit_layer.x==10:
                    game_ready=True
                    print("clicked on game board")

                if game_board_layer.collidepoint(event.pos) and game_ready:
                    row= normalise_selection(clicked_X,clicked_Y)[0]
                    col= normalise_selection(clicked_X,clicked_Y)[1]
                    print("gamedy", row,col)
                    if available_square(row,col):
                        pebble.play()
                        mark_square(row,col,player)
                ...
                if game_board_layer.collidepoint(event.pos) and time_attack:
                    row,col = normalise_selection(clicked_X,clicked_Y)
        ...
        draw_game_board(calc_alpha(scoreboard_layer),vert_line1_layer,vert_line2_layer,hor_line1_layer,hor_line2_layer)
